...
def new_event_tab(df, user_email, TRAINERS, TYPES, STATUSES, SOURCES, MEDIUMS, LOCATIONS,
    ...
    with st.form("add_form", clear_on_submit=True):
        ...
        if submitted:
            ...
            st.success(f"✅ {len(events_to_add)} event(s) added!")
            st.rerun()
    return df
    ...
    if len(selected_events) == 1:
        ...
        with op_tab1:
            with st.form("single_edit_form"):
                ...
                with c2:
                    edit_type = st.selectbox("Type", TYPES, index=TYPES.index(selected_event["Type"]))
                    edit_status = st.selectbox("Status", STATUSES[1:], index=STATUSES[1:].index(selected_event["Status"]))
                    edit_source = st.selectbox("Source", SOURCES[1:], index=SOURCES[1:].index(selected_event["Source"]))
                with c3:
                    ...
                    edit_medium = st.selectbox("Medium", MEDIUMS, index=MEDIUMS.index(selected_event["Medium"]))
                edit_location = st.selectbox("Location", LOCATIONS, index=LOCATIONS.index(selected_event["Location"]))
